...
// src/models/QueueType.ts
function fromUggQueueType(uggString) {
  const result = {
    ranked_flex_sr: 1 /* RankedFlex */,
...
// src/models/player/IMatchSummary.ts
function fromUggMatchSummary(ugg) {
  return {
    id: ugg.matchId,
...
// src/utils/math/gradientDescent.ts
function gradientDescent(f3, x0, learningRate = 0.01, tolerance = 1e-4, maxIters = 100) {
  let x2 = x0;
  let iter = 0;
  while (iter < maxIters) {
    const diff = learningRate * gradient(f3, x2);
...
function gradient(f3, x2, dx = 1e-4) {
  return (f3(x2) - f3(x2 - dx)) / dx;
}

// src/utils/math/factorial.ts
...
// src/utils/math/combinations.ts
function combinations(n, k) {
  return factorial(n) / (factorial(k) * factorial(n - k));
}

// src/utils/stats/binomial.ts
function binomialPmf(n, k, p) {
  return combinations(n, k) * p ** k * (1 - p) ** (n - k);
}

// src/utils/stats/normal.ts
function normalPdf(x2, mean, std) {
  return 1 / (mean * Math.sqrt(2 * Math.PI)) * Math.E ** (-1 / 2 * ((x2 - mean) / std) ** 2);
}

// src/lib/performance.ts
var performanceCache = /* @__PURE__ */ new Map();
function getMeanForWinrate(games) {
...
function getProbability(wins, games, winrate) {
  return binomialPmf(games, wins, winrate) * normalPdf(winrate, 0.5, 0.11);
}

// src/models/player/IPlayerChampionStats.ts
function createPlayerChampionStats(championId, matchSummaries, totalGames) {
  let wins = 0;
  let losses = 0;
  let kills = 0;
  let assists = 0;
...
function getPlayerChampionStats(matchSummaries) {
  const matchesByChampion = /* @__PURE__ */ new Map();
  for (const match of matchSummaries) {
  ...
  for (const [championId, matches] of matchesByChampion.entries()) {
    championStats[championId] = createPlayerChampionStats(championId, matches, matchSummaries.length);
  }
  return championStats;
}

// src/models/player/IRoleStats.ts
function createRoleStat(role, matchSummaries, totalGames) {
  let wins = 0;
  let losses = 0;
  let kills = 0;
  let assists = 0;
...
function createRoleStats(matchSummaries) {
  const matchesByRole = /* @__PURE__ */ new Map();
  for (const match of matchSummaries) {
...
function createPlayerData(regionId, summonerProfile, matchSummaries, rankScores) {
  ...
  return {
    ...
    championStats: getPlayerChampionStats(matchSummaries),
...
// src/api/queries.ts
var FETCH_MATCH_SUMMARIES_QUERY = `query FetchMatchSummaries(
    $championId: [Int],
    $page: Int,
    $queueType: [Int],
    $regionId: String!,
...
var FETCH_PROFILE_RANKS_QUERY = `query FetchProfileRanks(
    $summonerName: String!,
    $regionId: String!,
    $seasonId: Int!
  ) {
...
var UggApi = class {
  ...
  static async getMatchSummaries(summonerName, regionId, seasonId) {
    let matchSummaries = [];
    let currentPage = 1;
    let lastReponse = await this.getMatchSummaryPage(summonerName, regionId, seasonId, currentPage);
    currentPage++;
    matchSummaries = [...matchSummaries, ...lastReponse.matchSummaries];
    while (!lastReponse.finishedMatchSummaries) {
      lastReponse = await this.getMatchSummaryPage(summonerName, regionId, seasonId, currentPage);
      currentPage++;
      matchSummaries = [...matchSummaries, ...lastReponse.matchSummaries];
    }
    return matchSummaries;
  }
  static async getMatchSummaryPage(summonerName, regionId, seasonId, page = 1) {
    const body = {
    ...
    return json.data.fetchPlayerMatchSummaries;
  }
  static async getPlayerRanks(summonerName, regionId, seasonId) {
    const body = {
  ...
  static async getUggVersion() {
    ...
    const json = await res.json();
    const versions = Object.keys(json).map((str) => str.split("_").map((x2) => parseFloat(x2)));
    const sorted = versions.sort(this.sortVersionsDesc);
    return sorted[0].join(".");
  }
  static sortVersionsDesc(a, b) {
    if (a[0] !== b[0]) {
      return b[0] - a[0];
    } else {
      return b[1] - a[1];
  ...
  static async getChampionRankingData(patch) {
    const URL2 = `https://stats2.u.gg/lol/1.1/champion_ranking/world/${patch.split(".").slice(0, 2).join("_")}/ranked_solo_5x5/platinum_plus/1.5.0.json`;
    const res = await fetch(URL2);
    const json = await res.json();
    const data = [];
    for (const [role, stats] of Object.entries(json[0])) {
...
// src/api/RiotApi.ts
var BASE_URL2 = "http://ddragon.leagueoflegends.com/";
var RiotApi = class {
  static async getChampionData(patch) {
    const url = `${BASE_URL2}cdn/${patch}.1/data/en_US/champion.json`;
...
// src/models/static/IStaticDataset.ts
function createStaticDataset(patch, championData, rankingData) {
  const championStats = {};
  const rankingDataByChampion = /* @__PURE__ */ new Map();
  ...
  for (const champion of championData) {
    ...
    const rankingData2 = rankingDataByChampion.get(id);
    championStats[id] = {
      championId: id,
      name: champion.name,
      riotId: champion.id,
      wins: rankingData2.reduce((sum, x2) => sum + x2.wins, 0),
      matches: rankingData2.reduce((sum, x2) => sum + x2.matches, 0),
      roleStats: Object.fromEntries(rankingData2.map((r2) => [
        fromRoleString(r2.role),
        {
          role: fromRoleString(r2.role),
          wins: r2.wins,
          matches: r2.matches,
          counters: r2.counters.map((c) => ({
            championId: c[0],
            wins: c[1],
            matches: c[2]
          }))
...
async function main() {
  ...
  await fs2.writeFile("./public/static/dataset.json", JSON.stringify(staticDataset));
}
main();
